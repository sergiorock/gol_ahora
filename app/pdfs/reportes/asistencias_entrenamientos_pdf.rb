module Reportes
  class AsistenciasEntrenamientosPdf < BasePdf
    ...
    def render
      encabezado("Reporte de Asistencia — Entrenamientos", "#{@asistencias.count} inscripciones", @filtros)
      resumen
      detalle
      pie_de_pagina
      @document.render
    ...
    def detalle
      ...
      data = @asistencias.map do |a|
        ...
        [
          ...
          ent&.nombre || "—",
          ent&.personal_deportivo&.full_name || "—",
          a.user.full_name,
          a.present ? "Presente" : "Pendiente"
        ]
      end

      render_table(data, ["Fecha", "Entrenamiento", "Entrenador", "Alumno", "Asistencia"],
